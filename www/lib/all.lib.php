...
function latest_all($skin_dir='', $subject='', $rows=10, $subject_len=40, $sub_table='', $cache_time=1, $options='')
{
    ...
    if(!G5_USE_CACHE || $cache_fwrite) {
		...
		//$sql = " select * from {$tmp_write_table} where bo_table not in({$not_in}) and wr_5 = 'Y' order by wr_6 limit 0, {$rows} ";
		$bo = sql_query($sql);
		...
		$sql .= "{$sql_union}) X where {$options} wr_is_comment = 0 and wr_5 = 'Y' order by wr_6 limit 0, {$rows}";
