...
<form name="fwrite" id="fwrite" action="<?php echo $action_url ?>" onsubmit="return fwrite_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off" style="width:<?php echo $width; ?>">
    ...
    if ($is_notice || $is_html || $is_secret || $is_mail) { 
        $option = '';
        if ($is_notice) {
            $option .= PHP_EOL.'<li class="chk_box"><input type="checkbox" id="notice" name="notice"  class="magic-checkbox" value="1" '.$notice_checked.'>'.PHP_EOL.'<label for="notice"><span></span>공지</label></li>';
        }
        if ($is_html) {
            if ($is_dhtml_editor) {
                $option_hidden .= '<input type="hidden" value="html1" name="html">';
            } else {
                $option .= PHP_EOL.'<li class="chk_box"><input type="checkbox" id="html" name="html" onclick="html_auto_br(this);" class="magic-checkbox" value="'.$html_value.'" '.$html_checked.'>'.PHP_EOL.'<label for="html"><span></span>html</label></li>';
            }
        }
        if ($is_secret) {
            if ($is_admin || $is_secret==1) {
                $option .= PHP_EOL.'<li class="chk_box"><input type="checkbox" id="secret" name="secret"  class="magic-checkbox" value="secret" '.$secret_checked.'>'.PHP_EOL.'<label for="secret"><span></span>비밀글</label></li>';
            } else {
                $option_hidden .= '<input type="hidden" name="secret" value="secret">';
            }
        }
        if ($is_mail) {
            $option .= PHP_EOL.'<li class="chk_box"><input type="checkbox" id="mail" name="mail"  class="magic-checkbox" value="mail" '.$recv_email_checked.'>'.PHP_EOL.'<label for="mail"><span></span>답변메일받기</label></li>';
        }
    }
    echo $option_hidden;
    ?>
        ...
        <ul class="bbs_write_div_l">
            <div>
                <ul>
					...
					<li class="divmb-10">
                        <input type="text" name="wr_6" value="<?php echo $write['wr_6']; ?>" id="wr_6" class="bbs_w_inp w100" placeholder="순위">
                    </li>

					<li class="divmb-10">
                        <input type="text" name="wr_1" value="<?php echo $write['wr_1']; ?>" id="wr_1" class="bbs_w_inp w100" placeholder="개요">
                    </li>
					<li class="divmb-10">
                        <input type="text" name="wr_2" value="<?php echo $write['wr_2']; ?>" id="wr_2" class="bbs_w_inp w100" placeholder="기획의도">
                    </li>
					<li class="divmb-10">
                        <input type="text" name="wr_3" value="<?php echo $write['wr_3']; ?>" id="wr_3" class="bbs_w_inp w100" placeholder="출연">
                    </li>
					<li class="divmb-10">
                        <textarea name="wr_4" id="wr_4" rows="3" class="bbs_w_textarea w100" placeholder="간략설명"><?php echo $write['wr_4'] ?></textarea>
                    </li>
                    <li class="divmb-10">
                    <input type="checkbox" id="wr_5" name="wr_5" class="magic-checkbox" value="Y" checked=""> <label for="wr_5">메인출력..?</label>
                    </li>
                    ...
                    <li class="divmb-10">
                        <input type="text" name="wr_9" value="<?php echo $write['wr_9']; ?>" id="wr_9" class="bbs_w_inp w100" placeholder="출력순서재설정">
                    </li>
