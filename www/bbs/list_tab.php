...
$topgbn_html = "";
if ($is_file_exist) {
   include($board_skin_path."/topgbn.php"); // 2019-08-09 추가
   
   $is_topgbn = true;
   $topgbn_href = G5_BBS_URL.'/board.php?bo_table='.$bo_table;

   if(count($cattop) > 0){
      $topgbn_html .= "<li><a href='".$topgbn_href."'>전체</a></li>";
      
      foreach($cattop as $value){
         if($value['use'] == "Y"):
            $topgbn_html .= "<li><a href='".$topgbn_href.$value['link']."'>".$value['name']."</li>";
         endif;
      }
   }
}
...
include_once($board_skin_path.'/list.skin_tab.php');